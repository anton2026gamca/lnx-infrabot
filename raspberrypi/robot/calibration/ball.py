...
def add_ball_color_range(lower_hsv: tuple, upper_hsv: tuple, camera: str = "both") -> None:
    """Add an additional ball color range to the existing ranges."""
    existing_ranges = shared_data.get_ball_calibration(camera)
    new_ranges = existing_ranges + [(list(lower_hsv), list(upper_hsv))]
    shared_data.set_ball_calibration(new_ranges, camera=camera)
    save_calibration_data()
    logger.info(f"Ball color range added: lower={lower_hsv} upper={upper_hsv}")

def set_ball_color_ranges(ranges: list[tuple[tuple, tuple]], camera: str = "both") -> None:
    ...
    shared_data.set_ball_calibration(formatted_ranges, camera=camera)
    save_calibration_data()
    logger.info(f"Ball calibration set with {len(ranges)} ranges")

def get_ball_color_ranges(camera: str = "front") -> list[tuple[tuple, tuple]]:
...
def remove_ball_color_range(index: int, camera: str = "both") -> bool:
    ...
    if 0 <= index < len(ranges):
        ranges.pop(index)
        shared_data.set_ball_calibration(ranges, camera=camera)
        save_calibration_data()
        logger.info(f"Ball color range {index} removed")
        return True
    return False

def calibrate_ball_distance(current_ball_distance_mm: float, camera: str = "front") -> float:
    """
    ...
    """
    ball_data = shared_data.get_camera_ball_data_for_camera(camera)
    ...
    shared_data.set_camera_ball_calibration_constant(calibration_constant, camera=camera)
    save_calibration_data()
